#!/usr/bin/env python3
"""
HO-Procurements Activity Timeline Tracker Generator
=========================================================
Processes Borouge Excel files - 'HO-Procurements' sheet
with stage gate milestones and EP/LP/F/A date tracking.

Column Order (Output):
Activity ID | Activity Name | Stage Gate | Early Planning |
Late Planning | Actual Date (A) |
Duration Deviation | Timeline Flag
...
DEFAULT_OUTPUT_FILE = r"01-03-26\ho_procurements_tracker.xlsx"
# ============================================================================


class HOProcurementsProcessor:
    """Processes HO-Procurements sheet and generates Activity Timeline Tracker."""

    # Column mappings from source sheet
    COL_LEVEL_OR_ID = 1       # Col A - Level (L0-L6) or Activity ID
    COL_DELAYED_FLAG = 2      # Col B - "Delayed PO" / 0
    COL_ACTIVITY_NAME = 3     # Col C - WBS Code / Activity Name
    COL_ACTIVITY_CODE = 4     # Col D - Activity ID code
    COL_MILESTONE_DESC = 5    # Col E - WBS / Activity Name (milestone description)
    COL_STAGE_GATE = 9        # Col I - Stage Gate (EP/LP/F/A)

    # Milestone date columns
    COL_ISSUE_RFQ = 10        # Col J - Issue RFQ to Bidders
    COL_BID_EVAL = 11         # Col K - Bid Evaluation (Completion)
    COL_COMM_BID_EVAL = 12    # Col L - Commercial Bid Evaluation (Completion)
    COL_ISSUE_PO = 13         # Col M - Issue Purchase Order to Vendor

    # Data rows
    DATA_START_ROW = 12       # First L5/L6 EP row
    HEADER_ROW = 6

    # Milestone names (for output)
    MILESTONES = [
        ('Issue RFQ to Bidders', 10),
        ('Bid Evaluation (Completion)', 11),
        ('Commercial Bid Eval (Completion)', 12),
        ('Issue Purchase Order to Vendor', 13),
    ]
    ...
    def load_workbook(self):
        try:
            ...
            sheet_name = 'HO-Procurements'
            if sheet_name not in self.workbook.sheetnames:
                for name in self.workbook.sheetnames:
                    if 'ho' in name.lower() and 'procurement' in name.lower():
                        sheet_name = name
                        break
                else:
                    raise ValueError(
                        f"'HO-Procurements' sheet not found. "
    ...
    def extract_data(self):
        ...
        for row_idx in range(self.DATA_START_ROW, total_rows + 1):
            try:
                col1 = self._get_cell_value(row_idx, self.COL_LEVEL_OR_ID)
                ...
                # Get activity name from col 3
                name_str = str(activity_name or '').strip()
                code_str = str(activity_code or col1 or '').strip()
                ...
                if stage_gate_str == 'EP':
                    group_index += 1
                    current_activity_name = name_str
                    current_group_key = (name_str, group_index)
                    ...
                    # Update activity code if we got a real code from col 4
                    if activity_code and str(activity_code).strip():
                        pass  # Keep the EP code
                else:
                    # Stage gate row without a matching EP
                    group_index += 1
    ...
    def _create_milestone_records(self):
        ...
        for group_key in self.activity_order:
            ...
            for ms_name, ms_col in self.MILESTONES:
                # EP = Planned Start, LP = Planned End, A = Actual, F = Forecast
    ...
    def _calculate_deviation(self, planned_start_ep, planned_end_lp, actual_date_a, forecast_date_f=None):
        ...
        if not actual_date_a or not isinstance(actual_date_a, datetime):
            if forecast_date_f and isinstance(forecast_date_f, datetime):
                timeline_flag = "Forecast"
            else:
    ...
    def generate_output(self, output_file):
        ...
        new_ws.title = "HO Procurements Tracker"
        ...
        for row_offset, record in enumerate(self.data_records):
            ...
            if record['timeline_flag'] == 'Delayed':
                ...
                not_started_count += 1
            elif record['timeline_flag'] == 'Forecast':
        ...
        print(f"\n{'='*60}")
        print("HO-PROCUREMENTS - SUMMARY STATISTICS")
        print(f"{'='*60}")
...
def main():
    if len(sys.argv) >= 2:
        ...
        print("=" * 60)
        print("HO-PROCUREMENTS - TRACKER GENERATOR")
        print("=" * 60)
        print()
    else:
        print("=" * 60)
    ...
    try:
        ...
        processor = HOProcurementsProcessor(input_file)
