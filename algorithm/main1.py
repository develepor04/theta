#!/usr/bin/env python3
"""
EDDR Activity Timeline Tracker Generator - Final Version
=========================================================
Processes Borouge EDDR Excel files with stage gate milestones
and EP/LP/F/A date tracking.
...
DEFAULT_OUTPUT_FILE = r"01-03-26\p6_consideration_output_updated_v2.xlsx"
# ============================================================================


class EDDRProcessor:
    """Processes EDDR Excel files and generates Activity Timeline Tracker."""
    
    # Column mappings
    COL_ACTIVITY_CODE = 4
    COL_ACTIVITY_NAME = 10
    COL_ACTUAL_START = 16
    COL_STAGE_GATE = 23
    COL_START = 24
    COL_IDCS = 25
    ...
    def load_workbook(self):
        try:
            ...
            if 'EDDR' not in self.workbook.sheetnames:
                raise ValueError("'EDDR' sheet not found in workbook")
            
            self.eddr_sheet = self.workbook['EDDR']
            print(f"✓ EDDR sheet loaded: {self.eddr_sheet.max_row} rows × {self.eddr_sheet.max_column} columns")
    ...
    def extract_data(self):
        ...
        total_rows = self.eddr_sheet.max_row
        processed_count = 0
        group_index = 0
        current_group_key = None  # Track current (activity_code, group_index)
        
        # Collect data organized by activity groups (EP → LP → F → A)
        for row_idx in range(12, total_rows + 1):
            try:
    ...
    def _get_cell_value(self, row, col):
        try:
            return self.eddr_sheet.cell(row=row, column=col).value
        except:
            return None
    ...
    def _build_scurve_timeseries(self):
        ...
        Returns list of dicts, one per month:
          { month_str, month_date, ep_cum, lp_cum, actual_cum,
            ep_pct, lp_pct, actual_pct,
            last_ep_activity, last_lp_activity, last_actual_activity }
        """
        from collections import OrderedDict

        # Collect dated entries: (date, activity_id, activity_name)
        ep_entries = []
        lp_entries = []
        actual_entries = []
        ...
        # Determine full month range across all date types
        ...
        # The denominator for percentage: total EP entries (the full plan scope)
        total_planned = max(len(ep_entries), len(lp_entries), 1)

        # Build cumulative per month
        def _monthly_cumulative(entries, month_keys):
            """For sorted (date, label) pairs, compute cumulative count and last label per month."""
            cum = 0
            idx = 0
            result = {}  # month_key -> (cumulative_count, last_label)
            last_label = ''
        ...
        for mk in month_keys:
            ...
            # Stop actual line after the last month with real actual data
    ...
    def generate_output(self, output_file):
        ...
        new_ws.title = "Activity Timeline Tracker"
        ...
        # Column widths
        column_widths = {'A': 15, 'B': 50, 'C': 15, 'D': 22, 'E': 22, 'F': 18, 'G': 24, 'H': 15}
...
def main():
    if len(sys.argv) >= 2:
        ...
        print("="*60)
        print("EDDR ACTIVITY TIMELINE TRACKER GENERATOR")
        print("="*60)
        print()
    else:
        print("="*60)
    ...
    try:
        ...
        processor = EDDRProcessor(input_file)
